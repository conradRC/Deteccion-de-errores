...
/*
	...
	public void goForwardWithColors(double steps, Color[] colors, double changeEvery) {
		double oldx = x, oldy = y, missingSteps = steps - changeEvery;
		...
		StdDraw.line(oldx, oldy, x, y);
		if (missingSteps > 0.001)
			goForwardWithColors(missingSteps);
		
			
	}

	public void goForwardWithColors(double steps, Color[] colors) {
		double oldx = x, oldy = y, changeEvery = Math.ceil(steps / colors.length), missingSteps = steps - changeEvery;
		byte randomColor = (byte) (Math.random() * colors.length);
		try {
			setPenColor(colors[randomColor]);
		} catch (ArrayIndexOutOfBoundsException err) {
	...
	public void goForwardWithColors(double steps, double changeEvery) {
		Color[] colors = { StdDraw.BLACK, StdDraw.BLUE, StdDraw.CYAN, StdDraw.DARK_GRAY, StdDraw.GRAY, StdDraw.GREEN,
				StdDraw.LIGHT_GRAY, StdDraw.MAGENTA, StdDraw.ORANGE, StdDraw.PINK, StdDraw.RED, StdDraw.YELLOW,
				StdDraw.WHITE, StdDraw.BOOK_BLUE, StdDraw.BOOK_LIGHT_BLUE, StdDraw.BOOK_RED, StdDraw.PRINCETON_ORANGE };
		double oldx = x, oldy = y, missingSteps = steps - changeEvery;
		byte randomColor = (byte) (Math.random() * colors.length);
		try {
			setPenColor(colors[randomColor]);
	...
	public void goForwardWithColors(double steps) {
		Color[] colors = { StdDraw.BLACK, StdDraw.BLUE, StdDraw.CYAN, StdDraw.DARK_GRAY, StdDraw.GRAY, StdDraw.GREEN,
				StdDraw.LIGHT_GRAY, StdDraw.MAGENTA, StdDraw.ORANGE, StdDraw.PINK, StdDraw.RED, StdDraw.YELLOW,
				StdDraw.WHITE, StdDraw.BOOK_BLUE, StdDraw.BOOK_LIGHT_BLUE, StdDraw.BOOK_RED, StdDraw.PRINCETON_ORANGE };
		double oldx = x, oldy = y, changeEvery = steps / colors.length, missingSteps = steps - changeEvery;
		byte randomColor = (byte) (Math.random() * colors.length);
		try {
			setPenColor(colors[randomColor]);
		} catch (ArrayIndexOutOfBoundsException err) {
	...
	public void goBackwardWithColors(double steps, Color[] colors, double changeEvery) {
		...
		StdDraw.line(oldx, oldy, x, y);
		if (missingSteps > 0.001)
			goBackwardWithColors(missingSteps);
		else
			StdDraw.setPenColor();
	}
	...
	public static void main(String[] args) {
		Turtle turtle = new Turtle(-200, 0, 0);
		turtle.setCanvasSize(800, 400);
		turtle.setXscale(-200, 200);
		turtle.setYscale(-50, 50);
